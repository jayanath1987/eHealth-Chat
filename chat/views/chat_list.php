...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) 
 	...
 	for ($i=0; $i <count($user_list);++$i){
		...
		if ($my_id == $user_list[$i]["UID"]){
			...
			echo '<li>';
				echo '<a href="javascript:void(0);" onclick="open_profile();">';
			...
			echo '</li>';
                           echo '<li>';
				echo '<a href="'.site_url('form/edit/user_changepwd/'.$my_id.' ').'">';
			...
			echo '</li>';
			echo '<li>';
				echo '<a href="'.site_url("user/visits").'" >';
					echo '<span class="glyphicon glyphicon-briefcase" >&nbsp;';
					echo '</span>';
					
				echo 'Today\'s consultations ';
				if (isset($no_visits) &&($no_visits>=0)){
						echo '<span class="badge label-success">'.$no_visits.'</span>';
					}
				echo '</a>';
			echo '</li>';
		...
		echo '<li>';
			if (($user_list[$i]["Status"] == "ON_LINE")&&($my_id != $user_list[$i]["UID"])){
				echo '<a href="javascript:void(0);" onclick=load_chat("'.$user_list[$i]["UID"].'"); title="'.$user_list[$i]["FirstName"].' '.$user_list[$i]["FirstName"].'">';
					...
					echo '</span>';
					echo $user_list[$i]["Title"].' '.$user_list[$i]["FirstName"].' '.$user_list[$i]["OtherName"];
				echo '</a>';
			}
		echo '</li>';
	}
